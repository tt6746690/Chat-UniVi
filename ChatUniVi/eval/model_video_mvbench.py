...
from ChatUniVi.eval.video_encoding import read_frame_mod, read_gif_mod
...
class EvalDatasetMvBench(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(video_path):
            if data_type == 'video':
                video_frames, slice_len = _get_rawvideo_dec(video_path, self.image_processor, s=bound[0], e=bound[1], max_frames=MAX_IMAGE_LENGTH)
            elif data_type == 'gif':
...
mvbench_data_list = {
    ...
    "Counterfactual Inference": (
        "counterfactual_inference.json", "clevrer/video_validation/", "video", False),
}
...
def eval_model(args):
    ...
    for (idx, sample_set, video_frames, slice_len) in tqdm(dataloader):
        ...
        try:
            ...
            #     'mm_projector.weight.device': model.get_model().mm_projector.weight.device,
            #     'mm_projector.bias.device': model.get_model().mm_projector.bias.device,
            #     'device': device,
            #     'rank': args.gpu,
            #     'slice_len': slice_len,
