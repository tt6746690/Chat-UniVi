...
if [[ ! -f "$SAVE_DIR/answers-generic/merge.jsonl" || \
      ! -f "$SAVE_DIR/answers-temporal/merge.jsonl" || \
      ! -f "$SAVE_DIR/answers-consistency/merge.jsonl" ]]; then
    ...
    echo "$SAVE_DIR/answers-{generic, temporal, consistency}/merge.jsonl exists!"
fi
